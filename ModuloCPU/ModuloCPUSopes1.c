...
int iterate_init(struct seq_file *m, void *v)       
{
	...
	seq_printf(m, "Carnet: 201313982\n");
	seq_printf(m, "Nombre: Carlos Daniel Alonzo Jimenez\n");
	seq_printf(m, "Sistema Operativo: CentOS Linux 7 (Core)\n");


    for_each_process( task ){   
         
        seq_printf(m,"\nId: %d Nombre: %s Estado: %ld",task->pid, task->comm, task->state);
